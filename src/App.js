  ...
  if (schritt === "profil") {
    ...
    return (
      <Screen
        ...
        titel="Alles bereit!"
        untertitel="Deine Auswahl"
      >
        <div style={{
          backgroundColor: "#16213e", borderRadius: "12px",
          padding: "1.2rem", marginBottom: "1rem", width: "100%"
        }}>
          <Zeile emoji="👤" label="Typ" sub={zusammenfassung} />
          <Zeile emoji="🌙" label="Übernachtung" sub={uebernachtungLabel} />
        </div>
        <p style={{ color: "#606080", fontSize: "0.85rem", textAlign: "center" }}>
          Nächster Schritt: Profil ausfüllen & Karte 🗺️
        </p>
        <p style={{ color: "#404060", fontSize: "0.8rem", textAlign: "center", marginTop: "0.5rem" }}>
          (wird in der nächsten Session gebaut)
        </p>
        <button
          onClick={() => { setSchritt("start"); setProfil({ hauptmodus: null, typ: null, uebernachtung: null }); }}
          style={{ ...zurueckStyle, marginTop: "1.5rem", color: "#1D9E75", borderColor: "#1D9E75" }}
        >
          ↺ Neu starten
        </button>
      </Screen>
...
function Screen({ titel, untertitel, breadcrumb, zurueck, children }) {
  return (
    <div style={containerStyle}>
      ...
      {breadcrumb && (
        <p style={{ fontSize: "0.8rem", color: "#606080", margin: "0 0 4px" }}>{breadcrumb}</p>
      )}
      <h2 style={{ fontSize: "1.7rem", margin: "0 0 0.3rem" }}>{titel}</h2>
      {untertitel && (
        <p style={{ color: "#a0a0c0", fontSize: "0.9rem", margin: "0 0 1.5rem" }}>{untertitel}</p>
      )}
